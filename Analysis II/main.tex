...
\item If $ f $ has a local minimum (respectively local maximum), then $ Df(a)=0 $ and $ H_{f,a}(h) $ is positive (respectively negative) semi-definite.
  ...
  \end{enumerate}
\end{theorem}
\pf Let $ g(t)=f(a+th) $. We know that $ g $ is twice differentiable. Apply Taylor's theorem for one dimension, so we get that
\[
...
\]
Hence taking $ h\to 0 $ by continuity of $ D^2f $, we get that the first term goes to zero, hence $ E(h)=o(||h||^2) $.\qed
