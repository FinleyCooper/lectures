...
\]
\end{definition}
Consider the case where we have quadratic loss, so $ L(\theta,\delta) = (\theta-\delta)^2 $. Then we have that
\[
...
\begin{align*}
	\Lambda_X(H_0;H_1) &= \frac{(2\pi \sigma_0)^{-n/2} \exp\left(-\frac1{2\pi \sigma^2_0} \sum_{i=1}^n (X_i - \mu_1)^2\right)}{(2\pi \sigma_0)^{-n/2} \exp\left(-\frac1{2\pi \sigma^2_0} \sum_{i=1}^n (X_i - \mu_0)^2\right)}\\
			   &= \exp\left(\frac{\mu_1 - \mu_0}{\sigma^2_0} n\overline X + \frac{n(\mu_0^2 - \mu_1^2)}{2\sigma_0^2}\right).
\end{align*}
...
\par
Suppose that $ X=(X_1,\dots, X_n)\iid \mathcal N(\mu ,\sigma_0^2) $ where $ \sigma_0 $ is known. We found a $ 100(1-\alpha)\% $ confidence interval for $ \mu $, namely, $ I(X)=\overline X \pm \frac{\sigma_0}{\sqrt n} $. Using the second part of the theorem, we can find a size $ \alpha $ test for $ H_0: \mu = \mu_0 $, by defining the acceptance region as
\begin{align*}
...
Suppose that $ X_1,\dots, X_n\iid \mathcal N(\mu,\sigma^2_0) $ with $ \sigma_0^2 $ known. We wish to test
\begin{align*}
	H_0&: \mu = \mu_0\\
	H_1&: \mu \ne \mu_0.
\end{align*}
...
\begin{example}
  ...
  Suppose $ X_1,\dots, X_n $ are iid samples from a distribution on $ \{ 1,\dots, k\ $. Let $ p_i = \mathbb P(X_1 = i) $ and let $ N_i $ be the number of observations in $ \{X_1,\dots, X_n\} $ of type $ i $. Hence $ \sum p_i = 1 $ and $ \sum N_i = n $. Then we have that
	  ...
	  $ 2\log \Lambda\approx \chi_p^2 $ with $ p=\dim(\Theta_1)-\dim(\Theta_0) = k-1 - 0 = k-1 $. Thus reject $ H_0 $ if $ 2\log \Lambda\ge \chi_{k-1}^2(\alpha) $. It is common to write
	  \[
...
\begin{example}
  ...
  \]
  Writing $ o_{ij} =N_{ij} $ and $ e_{ij} = n\hat p_{i+}\hat p_{+j} $ we have that
  \[
...
\begin{example}
  ...
  \]
  Under Lagrangian methods with constraints $ \sum_j p_{ij} = 1 $ we find the MLE
  \[
...
\]
And Wilks' theorem states that asymptotically the distribution of $ 2\log\Lambda $ is $ \chi^2_p $ with 
\[
...
Consider the Cambridge admissions stats in 1996.
\begin{center}
	\begin{tabular}{c | c c c | c c c}
\hline
Subject & Applied (F) & Accepted (F) & \% Success & Applied (M) & Accepted (M) & \% Success \\
\hline
CS & 26 & 7 & 27 & 228 & 58 & 25 \\
Econ & 240 & 63 & 26 & 512 & 112 & 22 \\
Eng & 164 & 52 & 32 & 972 & 252 & 26 \\
Med & 416 & 99 & 24 & 578 & 140 & 24 \\
Vet Med & 338 & 53 & 16 & 180 & 22 & 12 \\
\hline 
Total & 1184 & 274 & 23 & 2470 & 584 & 24\\
\hline
  \end{tabular}
\end{center}
We can see that the men had a worse success rate than the women for all subjects. However averaging over the total, the men have a better success rate. The is \textit{Simpson's paradox} and occurs since the men apply for 'easier' subjects.
...
\begin{proposition}
  A multivariate normal distribution is fully specified by its mean and covariance.
\end{proposition}
\pf Let $ X_1 $ and $ X_2 $ both be multivariate normal with mean $ \mu\in \R^n $ and covaraince $ \Sigma\in \R^{n\times n } $. We will show that their mgfs are equal, hence their distributions are the same.
\begin{align*}
	M_{X_1}(t) = \mathbb E[e^{t^TX_1}] &= \exp\left(\mathbb E[t^TX_1] + \frac 12 \Var{t^TX_1}\right)\\
					   &=\exp\left(t^T\mu + \frac 12 t^T \Sigma t\right).
\end{align*}
Since the calculation is the same for $ X_2 $, they have the same distribution.\qed
\begin{corollary}
	Suppose $ X_1,\dots, X_n $ are indepdently distributed with distributions $ \mathcal N(\mu_i,\sigma^2) $ respectively. Let $ A\in \R^{n\times n} $ be an orthogonal matrx, so $ A^TA=AA^T = I $. Then the elements of $ Y=AX\in \R^n $ are independently distributed, and $ Y_i\sim\mathcal N((A\mu)_i,\sigma^2) $, where \[\mu=\begin{pmatrix}
	  \mu_1 \\
	  \vdots \\
	  \mu_n
  \end{pmatrix}. \]
\end{corollary}
\pf The distribution of $ Y $ is determined by the mean vector and the covariance matrix. 
\[
	\mathbb E[Y] = \mathbb E[AX] = A\mathbb E[X] = A\mu
\]
and
\[
	\Cov{AX} = A\Cov{X}A^T=A(\sigma^2 I)A^T = \sigma^2 I.
\]
\qed
\par
Suppose we have normal observations with unknown mean and variance, $ X_1,\dots, X_n \iid \mathcal N(\mu,\sigma^2) $ for some $ \mu\in \R $ and $ \sigma^2>0 $. Recall that the MLE of $ \mu $ is 
\[
	\overline X = \frac 1n \sum_{i=1}^n X_i
\]
and the MLE of $ \sigma^2 $ is 
\[
	\hat\sigma^2 = \frac{S_{xx}}n = \frac{\sum_{i=1}^n (X_i-\overline X)^2}n.
\]
\begin{theorem}
  Suppose $ X_1,\dots, X_n \iid \mathcal N(\mu,\sigma^2)$. Then
  \begin{enumerate}
  ...
  \end{enumerate}
\end{theorem}
\pf The first part is an easy consequence of the fact that linear combinatiosn of Gaussians are Gaussian. For the second and third parts we write that
\begin{align*}
	\sum_{i=1}^n (X_i-\mu)^2 &= \sum_{i=1}^n((X_i-\overline X)+(\overline X-\mu))^2 \\
				 &= \sum_{i=1}^n ((X-\overline X)^2 + 2(X_i - \overline X)(\overline X-\mu) +(\overline X - \mu)^2) \\
				 &= S_{xx} + n(\overline X-\mu)^2
\end{align*}
Let $ A $ be an orthogonal matrix such that
\[
  Y=A\left(X-\begin{pmatrix}
    \mu\\
    \vdots \\
    \mu
...
\[
  ...
  \end{pmatrix}
\]
where the rows below the first are chosen so $ A\in O(n) $.
Then $ Y_1 = \sqrt n(\overline X-\mu) \sim\mathcal N(0,\sigma^2) $ and $ Y_1 $ is independent of $ (Y_2,\dots, Y_n) $ by the corollary. Since
\begin{align*}
	\sum_{i=1}^n Y_i^2 &= \sum_{i=1}^n (X_i-\mu)^2 \\
	||Y||_2^2 &= \left|\left|A(X-\begin{pmatrix}
	  \mu \\
	  \vdots \\
	  \mu
...
\]
thus $ S_{xx} $ is independent of $ Y_1 $ thus also independent of $ \overline X $. Also
\[
	\frac{S_{xx}}{\sigma^2} = \sum_{i=2}^n \frac{Y_i^2}{\sigma^2}
\]
has a $ \chi_{n-1}^2 $ distribution.\qed
\subsection{$t$-Tests}
\begin{definition}
  If $ U\sim\mathcal N(0,1) $ and $ V\sim\chi_n^2 $ with $ U $ independent of $ V $ then
  \[
	  T= \frac U{\sqrt{V/n}}
  \]
  has a \textit{Student's} $ t $\textit{-distribution} with $ n $ degrees of freedom, denoted by $ t_n $.
\end{definition}
\begin{example}
  Suppose that $ X_1,\dots, X_n\iid \mathcal N(\mu,\sigma^2) $. When $ \sigma^2 $ is knonw, we find a $ 100(1-\alpha)\% $ confidence interval for $ \mu $ by noting that 
  \[
	  Z = \frac{\sqrt n (\overline X - \mu)}\sigma\sim\mathcal N(0,1)
  \]
  so $ \overline X \pm z_{\alpha/2}\frac\sigma{\sqrt n} $ is the required confidence interval. 
\end{example}
\begin{example}
	If instead $ \sigma^2 $ is unknown, since $ \overline X $ and $ \hat \sigma^2 = \frac{S_{xx}}n $ are independent with $ \overline X \sim\mathcal N(\mu,\frac{\sigma^2}n) $ and $ S_{xx}\sim \chi_{n-1}^2 $. We have the pivot
	\[
		T = \frac{\sqrt n (\overline X -\mu)/\sigma}{\sqrt{S_{xx}/\sigma^2(n-1)}}\sim t_{n-1}.
	\]
	Thus
	\begin{align*}
		\mathbb P_{\mu,\sigma^2}\left(-t_{n-1}\left(\frac \alpha 2\right) \le \frac{\sqrt n(\overline X -\mu)}{\sqrt{S_{xx}/(n-1)}} \le t_{n-1}\left(\frac \alpha2\right)\right) = 1-\alpha
	\end{align*}
	so
	\[
		\overline X\pm \sqrt{\frac{S_{xx}}{n-1}}\cdot \frac{t_{n-1}\left(\frac \alpha 2\right)}{\sqrt n}
	\]
	is an exact $ 100(1-\alpha)\% $ CI for $ \mu $.
\end{example}
\begin{example}
  If instead we wish to test $ H_0:\mu = \mu_0 $ with $ \sigma^2 $ unknown, then under $ H_0 $ we have that
  \[
	  T = \frac{\sqrt n(\overline X - \mu_0)}{\sqrt{S_{xx}/(n-1)}}\sim t_{n-1}
  \]
  so reject $ H_0 $ when $ |T|\ge t_{n-1}\left(\frac \alpha 2\right) $. This is a $ t $-test.
\end{example}
\begin{remark}
	As $ n\to\infty $, $ \frac{S_{xx}}{n-1}\to \sigma^2 $ and $ t_{n-1}\left(\frac \alpha 2\right) \to z_{\alpha /2} $. Then the $ t $-test converges to the $ z $-test. Importantly the validity of the $ t $-test does not need large $ n $.
\end{remark}
\begin{remark}
  The $ t $-test can be seen as an example of a GLRT for 
  \begin{align*}
	  H_0 &: \mu = \mu_0 \\
	  H_1 &: \mu \ne \mu_0.
  \end{align*}
  The GLR is
  \begin{align*}
	  \Lambda_X(H_0;H_1)=\frac{\sup_{\mu,\sigma^2} f(X\mid \mu,\sigma^2)}{\sup_{\sigma^2} f(X\mid \mu_0,\sigma^2)}
  \end{align*}
  Plugging in $ (\hat \mu, \hat \sigma^2)= (\overline X, \frac{S_{xx}}n) $ the numerator is $ \left(\frac{2\pi \sum(X_i-\overline X)^2}{n}\right)^{-n/2}\exp\left(-\frac n2\right) $. When $ \mu = \mu_0 $ the MLE for $ \sigma^2 $ is $ \sum\frac{(X_i-\mu_0)^2}n $. The denominator is $ (\frac{2\pi \sum(X_i-\mu_0)^2}{n})^{-n/2}\exp(-\frac n2) $. Then
  \begin{align*}
	  \Lambda_X(H_0;H_1) &= \frac{(2\pi \sum(X_i-\overline X)^2)^{-n/2}}{(2\pi \sum(X_i-\mu_0)^2)^{-n/2}}\\
			     &= \left(1+\frac{n(\overline X-\mu_0)^2}{\sum (X_i-\overline X)^2\right)^{n/2}}.
  \end{align*}
  Thus reject $ H_0 $ when $ \frac{\sqrt n(\overline X-\mu_0)}{\sqrt{S_{xx}/(n-1)}} $ is large.
\end{remark}
\subsection{Analysis of variance}
\begin{example}
  Cambridge Part IA 2025 exam results
  \begin{center}
	  \begin{tabular}{|c | c | c c c|}
		  \hline
		  College & Total Marks & Probability & Differential Equations & Vector Calculus \\
		  \hline
		  Trinity & 318 & 36 & 29 & 32 \\
		  Churchill & 283 & 25 & 37 & 35 \\
		  St Johns & 294 & 28 & 35 & 37 \\
		  Clare & 244 & 15 & 39 & 31 \\
		  Christ's & 280 & 24 & 34 & 35 \\
      \hline
    \end{tabular}
  \end{center}
  We consider testing equality of means among $ k>2 $ groups. Our model is
  \[
	  X_{ij} = \mu_i +\ep_{ij}, \quad\forall 1\le j \le n_i \quad 1\le i \le k
  \]
  where $ n_i $ is the number of observations for the $ i $th group. Let $ N=\sum_{i=1}^k n_i $. We assume that $ \ep_{ij}\iid \mathcal N(0,\sigma^2) $.
  \begin{align*}
	  H_0&:\mu_1 = \mu_2 =\dots = \mu_k \\
    H_1&: H_0 \ \text{is false}
  \end{align*}
  We use a GLRT with ratio of likelihoods being
  \begin{align*}
  \Lambda_X(H_0;H_1) &=\frac{\sup_{\mu_1,\dots, \mu_k,\sigma^2}(2\pi \sigma^2)^{-N/2}\exp\left(-\sum_{i,j}\frac{(X_{ij}-\mu_i)^2}{2\sigma^2}\right)}{\sup_{\mu,\sigma^2}(2\pi\sigma^2)^{-N/2}\exp\left(-\sum_{i,j}\frac{(X_{ij}-\mu)^2}{2\sigma^2}\right)}\\
		     &=\left(\frac {s_0}{s_1}\right)^{N/2}
  \end{align*}
  where
  \begin{align*}
	  s_0 &= \sum_{i,j}(X_{ij}-\overline X_{\cdot\cdot})^2\\
	  s_1 &= \sum_{i,j}(X_{ij}-\overline X_{i\cdot})^2
  \end{align*}
  where
  \begin{align*}
	  \overline X_{\cdot\cdot} &= \frac{\sum_{i,j}X_{ij}}{N}\\
	  \overline X_{i\cdot} &= \frac 1{n_i}\sum_{j=1}^{n_i}X_{ij}.
  \end{align*}
  We should reject $ H_0 $ is $ \frac{s_0}{s_1} $ is large. Note that
  \begin{align*}
	  s_0 &= \sum_{i=1}^k\sum_{j=1}^{n_i} (X_{ij}-\overline X_{i\cdot} + \overline X_{i\cdot} - \overline X_{\cdot\cdot})^2 \\
	      &=\underbrace{\sum_{i,j}(X_{ij}-\overline X_{i\cdot})^2}_{s_1}+\underbrace{\sum_i n_i(\overline X_{i\cdot} - \overline X_{\cdot\cdot})^2}_{s_2}
  \end{align*}
  We call $ s_1 $ the within samples sum of squares and $ s_2 $ the between sample sum of squares. We reject $ H_0 $ if $ \frac{s_2}{s_1} $ is large.\par
  Note that $ \sum_j(X_{ij}-\overline X_{i\cdot})^2 $ is independent of $ \overline X_{i\cdot} $, implying that $ s_1 $ is independent of $ s_2 $ (since the latter is a function of $ \{\overline X_{i\cdot}\}_i $ and the observations are independent for different $ i $. Also if $ H_9 $ is true
  \[
	  s_1\sim \sigma^2 \chi^2_{N-k} \quad \text{and} \quad s_2 \sim \sigma^2 \chi^2_{k-1}.
  \]
\begin{definition}
	($ F $-distribution) If $ V\sim \chi^2_n $ and $ W\sim \chi^2_m $ with $ V $ and $ W $ independent, we say that
	\[
		F = \frac{V/n}{W/m}
	\]
	has an $ F $ distribution with $ (n,m) $ degrees of freedom.
\end{definition}
Thus under $ H_0 $ we have that
\[
	\frac{S_2/(k-1)}{S_1/(N-k)} \sim F_{k-1,N-k}
\]
and we obtain a size $ \alpha $ test if we reject $ H_0 $ when the test statistic exceeds $ F_{k-1,N-k}(\alpha) $.
\begin{remark}
  Recall the $ t $-distribution corresponding to
  \[
	  T = \frac{U}{\sqrt{V/n}}
  \]
  where $ U\sim \mathcal N(0,1) $ and $ V\sim \chi^2_n $. Since $ U^2\sim \chi_1 $, the statistic
  \[
	  T^2 = \frac{U^2}{V/n}\sim F_{1,n}.
  \]
\end{remark}
For our exam data we have $ k=5 $ and $ N=100 $, so we're looking at the distribution $ F_{4,95} $. At the $ 5\% $ significance level we reject $ H_0 $ if the test statistic $ \ge F_{4,95}(0.05)\approx 2.47 $. Our corresponding $ F $-statistics are the following
\begin{center}
	\begin{tabular}{|c | c c c c|}
		\hline
		& Total Marks & Probability & Differential Equations & Vector Calculus \\
		\hline
		$ F $-statistic & 3.71 & 4.25 & 1.70 & 0.5 \\
    \hline
  \end{tabular}
\end{center}
So we reject $ H_0 $ for the total marks and Probability only.
\begin{remark}
	What we described is called \textit{one-way} ANOVA. Instead we could have done \textit{two-way} ANOVA which analyses data defined according to two varaibles. For example we could investigate students' performance on exams depends on how many supervisions were given and which college they were from. Our model this time would be
	\[
		Y_{ijk}= \alpha + \mu_j + \lambda_k + \ep_{ijk}
	\]
	where $ \ep_{ijk} $ is the binary coding of supervisions/college.
\end{remark}
\end{example}
\section{Linear models}
\subsection{Definitions}
Suppose we have data $ \{(x_1,Y_1,\dots, (x_n,Y_n)\} $ with $ Y_i\in \R $ and $ x_i\in \R^p $. In a \textit{linear model} we assume that 
\[
	Y_i = \alpha + \beta_1 x_{i1} + \beta_2 x_{i2}+\dots +\beta_p x_{ip} + \ep_i
\]
where $ \alpha $ is the \textit{intercept}, $ (\beta_1,\dots, \beta_p) $ are \textit{regression coefficients}, and $ (\ep_1,\dots, \ep_n) $ are random "noise" with mean $ 0 $ and variance $ \sigma^2 $.\par
We want to estimate the parameters $ (\alpha,\beta_1,\dots, \beta_p) $. We call the $ x_i $'s the predictors or explanatory variables and the $ Y_i $'s response variables.
\begin{remark}
	We often eliminate the intercept by definining the first predictor $ x_{i1}=1 $ for all $ i $, so $ \beta_1 $ plays the role of $ \alpha $.
\end{remark}
\begin{remark}
	We can also model non-linear relationships. For example $ x_{i1} = z_i $ and $ x_{i2}= z_i^2 $ so the "linear" model is then $ Y_i = \alpha + \beta_1z_1+\beta_2z_i^2+\ep_i $.
\end{remark}
\begin{remark}
The coefficient $ \beta_j $ can be interpreted as the effect on the response of increasing $ x_{ij} $ by one unit by keeping other predictors fixed. In general, however, linear models can only reveal correlations in data rather than causation.
\end{remark}
The matrix formulation of a linear model if
\[
  Y=\begin{pmatrix}
    Y_1 \\
    \vdots \\
  ...
  \end{pmatrix},\quad \beta = \begin{pmatrix}
    ...
    \vdots \\
    \beta_p
  \end{pmatrix},\quad \ep = \begin{pmatrix}
    \ep_1 \\
    \vdots \\
    \ep_n
  \end{pmatrix}.
...
\begin{enumerate}
	\item $ \mathbb E(\ep) = 0 $, so $ \mathbb E(\ep_i) = 0 $ for all $ i $ and $ \mathbb E(Y_i) = x_{i1}\beta_1+\cdots + x_{ip}\beta_p $.
	\item $ \Cov{\ep} = \sigma^2 I $ so $ \Var{\ep_i} = \sigma^2 $ for all $ i $, $ \Cov{\ep_i,\ep_j} = 0 $ for all $ i\ne j $, $ \Var{V_i} = \Var{x_i^T\beta+\ep_i} =\Var{\ep_i} = \sigma^2 $.
\end{enumerate}
For now, we don't assume anything else about the distribution of the $ \ep_i $'s. Throughout out discussion we assume that $ X\in \R^{n\times p} $ is fixed with $ p\le n $ and $ \mathrm{rank}(X)=p $.
\subsection{Least squares estimator}
The \textit{least squares estimator} $ \hat\beta $ minimises the residual sum of squares.
\begin{align*}
	S(\beta) = ||Y-X\beta||_2^2 &= \sum_{i=1}^n (Y_i-X_i^T\beta)^2
\end{align*}
where $ X_i^T $ is the $ i $th row of $ X $. This is a quadratic polynomial in $ (\beta_1,\dots, \beta_p) $, so $ \hat\beta $ satisfies
\[
	\left.\frac{\partial S(\beta)}{\partial \beta_k}\right|_{\beta=\hat \beta} =0
\]
for all $ 1\le k\le p $. Thus we have that
\[
	-2\sum_{i=1}^n x_{ik}\left(Y_i-\sum_{j=1}^p x_{ij}\hat \beta_j\right)=0
\]
for all $ 1\le k \le p $. Or in matrix form,
\[
...
\]
\begin{remark}
  The least squares estimator is linear, so $ \hat\beta = MY $ for some fixed matrix $ M $. We can check:
  \begin{enumerate}
	  \item  $\mathbb E[\hat\beta] = (X^TX)^{-1}X^T\mathbb E[Y] = \beta $ \quad ($ \hat\beta $ is unbiased).
	  \item  $ \mathrm{Cov}{\hat\beta}= (X^TX)^{-1}X^T\Cov{Y}X(X^TX)^{-1} =\sigma^2(X^TX)^{-1} $.
  \end{enumerate}
\end{remark}
\begin{theorem}
(Gauss-Markov) Let $ \beta^* = CY $ be any other unbiased linear estimator of $ \beta $. Then for any $ t\in \R^p $, $ \mathrm{Var}(t^T\hat\beta) \le \mathrm{Var}(t^T\beta^*) $. Thus the least squares estimator is the best linear unbiased estimator.
\end{theorem}
\pf Note that the condition $ \mathrm{Var}(t^T\beta^*) - \mathrm{Var}(t^T\hat\beta) = t^T(\mathrm{Cov}(\beta^*) - \mathrm{Cov}(\hat \beta))t\ge 0 $, is equivalent to $ \mathrm{Cov}(\beta^*) - \mathrm{Cov}(\hat\beta) $ being positive semidefinite.\par
Let $ A = C-(X^TX)^{-1}X^T $. Then $ \mathbb E(AY) = \mathbb E(\beta^*)-\mathbb E(\hat\beta) = 0 $. Since $ \mathbb E(AY) = A\mathbb E(Y) =AX\beta$ for all $ \beta\in \R^p $, this means that $ AX=0 $. Furthermore,
\begin{align*}
	\mathrm{Cov}(\beta^*) &= \mathrm{Cov}((A+(X^TX)^{-1}X^T)Y) \\
			      &= \sigma^2(AA^T +(X^TX)^{-1}+AX(X^TX)^{-1}+(X^TX)^{-1}X^TA^T) \\
			      &= \sigma^2 AA^T + \mathrm{Cov}(\hat \beta).
\end{align*}
So $ \mathrm{Cov}(\beta^*) - \mathrm{Cov}(\hat\beta) = \sigma^2 AA^T  $ is positive semidefinite.\qed
\begin{remark}
  If we think about $ (t_1,\dots, t_p) $ as the values of the predictors for a new sample, $ t^T\hat \beta $ and $ t^T\beta^* $ would be our predictions for the mean response using either estimator. The Gauss-Markov theorem says that $ t^T\hat\beta $ always gives a smaller variance, and since $ t^T\hat\beta $ and $ t^T\beta^* $ are unbiased the MSE is also smaller.
\end{remark}
\begin{definition}
	We say that $ P\in \R^{n\times n} $ is an \textit{orthogonal projection} if it is idempotent and symmetric.
\end{definition}
\begin{proposition}
	We have the following:
	\begin{enumerate}
		\item The matrix $ P $ is an orthogonal projection if and only if for all $ v\in \mathrm{col}(P) $ we have that $ Pv=v $ and for all $ w\in \mathrm{col}(P)^\perp $ we have that $ Pw = 0 $. Equivalently we can write $ P = UU^T $ where the columns of $ U\in \R^{n\times k} $ form an orthonormal basis of $ \mathrm{col}(P) $ and $ k=\mathrm{rank}(P) $.
		\item If $ P $ is an orthogonal projection, then so is $ I-P $.
		\item $ ||Y||_2^2 = ||PY||_2^2 +||(I-P)Y||_2^2 $ for any $ Y\in \R^ n $.
		\item The eigenvalues of $ P $ are all $ 0 $ or $ 1 $. In particular $ \tr(P)= \mathrm{rank}(P) $.
		\end{enumerate}
\end{proposition}
The fitting values in linear regression are $ \hat Y = X\hat \beta $ (predicted mean responses) and the residuals are $ Y-\hat Y $. Note that 
\begin{align*}
	\hat Y =\underbrace{ X(X^TX)^{-1} X^T}_P Y,
\end{align*}
where we call $ P $ the \textit{hat matrix}, and $ Y-\hat Y = (I-P)Y $.
\begin{proposition}
	$ P $ is the orthogonal projection onto $ \mathrm{col}(X) $.
\end{proposition}
\pf $ P $ is clearly symmetric and $ P^2 = X(X^TX)^{-1}X^TX(X^TX)^{-1}X^T = P $ so $ P $ is an orthogonal projection. Also we have that $ \mathrm{col}(P) = \mathrm{col}(X) $ since for any $ a\in \R^n $ we have that
\[
	Pa =X(X^TX)^{-1}X^T a \in \mathrm{col}(X),
\]
and conversely if $ b = Xc \in \mathrm{col}(X) $, then
\[
	b = Xc = X(X^TX)^{-1}(X^TX)c = Pb \in \mathrm{col}(P).\qed
\]
\begin{corollary}
	The vector of fitting values $ \hat Y = PY $ is the orthogonal projection of $ Y $ onto $ \mathrm{col}(X) $ and the vector of residuals $ Y-\hat Y = (I-P)Y $ is the orthogonal projection of $ Y $ onto $ \mathrm{col}(X)^\perp $.
\end{corollary}
\subsection{Maximum likelihood estimation}
In addition to $ \mathbb E[\ep] = 0 $ and $ \mathrm{Cov}(\ep) = \sigma^2 I $ we now assume that $ \ep $ is multivariate normal, so $ \ep \sim \mathcal N(0,\sigma^2 I) $. We wish to estimate the parameters $ (\beta,\sigma^2) $. The observations of $ Y $ have pdf
\[
	f_Y(Y\mid \beta,\sigma^2) = (2\pi \sigma^2)^{-n/2}\exp\left(-\frac 1{2\sigma^2}\sum_{i=1}^n(Y_i-X_i^T\beta)^2\right).
\]
The log-likelihood is
\[
	\ell(\beta,\sigma^2) = \text{const.} - \frac n2 \log\sigma^2 - \frac 1{2\sigma^2}||Y-X\beta||_2^2.
\]
Note that for any $ \sigma^2>0 $, $ \ell(\beta,\sigma^2) $ is maximised when $ S(\beta) $ is minimised. Thus the MLE of $ \beta $ is the least squares estimator $ \hat\beta $. The MLE for $ \sigma^2 $ minimises
\[
	\ell(\hat\beta,\sigma^2) = \text{const.} - \frac n2 \log\sigma^2 - \frac {1}{2\sigma^2}S(\hat\beta).
\]
So
\[
	\frac{\partial \ell}{\partial \sigma^2} = -\frac n{2\sigma^2} + \frac 1{2\sigma^4} S(\hat\beta).
\]
which setting to zero gives that
\[
	\hat\sigma^2 = \frac{S(\hat\beta)}n = \frac{||Y-X\hat\beta||^2_2}n = \frac{||Y-\hat Y||_2^2}n = \frac{||(I-P)Y||_2^2}n.
\]
\begin{theorem}
  If $ X\sim \mathcal N(0,\sigma^2 I) $ and $ P $ is an orthogonal projection matrix, then 
  \begin{enumerate}
	  \item $ PX\sim \mathcal N(0,\sigma^2 P) $ and $ (I-P)X\sim \mathcal N(0,\sigma^2(I-P)) $ are independent;
	  \item $ \frac{||PX||^2_2}{\sigma^2} \sim\chi_{\mathrm{rank}(P)}^2 $.
  \end{enumerate}
\end{theorem}
\pf Part (i) proved on Example Sheet 3. For part $ 2 $ note that
\[
	\frac{||PX||_2^2}{\sigma^2} = \frac{(PX)^T(PX)}{\sigma^2} = \frac{X^T(UU^T)^T(UU^T)X}{\sigma^2}
\]
where the columns of $ U $ form a basis for $ \mathrm{col}(P) $. This then equal to
\[
	\frac{||U^TX||_2^2}{\sigma^2} = \sum_{i=1}^{\mathrm{rank}(P)}\frac{(U^TX)_i^2}{\sigma^2}.
\]
But we know that $ U^TX \sim\mathcal N(0,\sigma^2 U^TU) = \mathcal N(0,\sigma^2 I) $, so $ \frac{(U^TX)_i}{\sigma} $ are iid $ \mathcal N(0,1) $ for all $ i $. Hence
\[
	\frac{||PX||_2^2}{\sigma^2}\sim\chi^2_{\mathrm{rank}(P)} = \chi^2_{\mathrm{tr}(P)}.
\]
\begin{corollary}
  We have that 
  \begin{enumerate}
	  \item $ \hat\beta\sim \mathcal N(\beta,\sigma^2(X^TX)^{-1}) $
	  \item $ \frac{\hat\sigma^2}{\sigma^2}n\sim \chi^2_{n-p} $
	  \item $ \hat\beta $ and $ \hat\sigma^2 $ are independent.
  \end{enumerate}
\end{corollary}
\pf For (i) we already know that $ \mathbb E[\hat \beta] = \beta $ and $ \mathrm{Cov}(\hat\beta) = \sigma^2(X^TX)^{-1} $. Since $ \hat\beta = (X^TX)^{-1}X^TY $ is linear in $ Y\sim \mathcal N(X\beta, \sigma^2 I) $ we know that $ \hat\beta $ is also multivariate normal.\par
For (ii) note that
\[
	\frac{\hat \sigma^2}{\sigma^2}n = \frac{||(I-P)Y||^2_2}{\sigma^2} = \frac{||(I-P)(X\beta+\ep)||^2_2}{\sigma^2} = \frac{||(I-P)\ep||_2^2}_{\sigma^2}\sim \chi^2_{\mathrm{tr}(I-P)}.
\]
Note that $ \tr(I-P) = \tr(I)-\tr(P) = n - p $.
\par
For (iii) note that $ \hat\sigma^2 $ is a functoin of $ (I-P)\ep $ and
\begin{align*}
	\hat\beta &= (X^TX)^{-1} X^TY = (X^TX)^{-1} X^{-1}(X\beta+\ep) \\
		  &= \beta + (X^TX)^{-1}X^T\ep\\
		  &= \beta + (X^TX)^{-1} X^T P\ep
\end{align*}
using the fact that $ P=X(X^TX)^{-1}X^T $. So $ \hat\beta $ is a function of $ P\ep $. By the previous theorem we have that $ \hat\beta $ and $ \hat\sigma^2 $ are independent.\qed
\par
Suppose we want a $ 100(1-\alpha)\% $ confidence interval for the coefficients in the linear model. \textit{Wlog} consider just $ \beta_1 $. Note that
\[
	\frac{\hat \beta_1 - \beta_1}{\sqrt{\sigma^2(X^TX)^{-1}_{11}}}\sim \mathcal N(0,1)
\]
independent of $ \frac{\hat\sigma^2}{\sigma^2} n \sim \chi^2_{n-p} $. We use this to construct a pivot since
\[
	\frac{\frac{\hat \beta_1 - \beta_1}{\sqrt{\sigma^2(X^TX)^{-1}_{11}}}}{\sqrt{\frac{\hat\sigma^2 n}{\sigma^2 (n-p)}}}\sim t_{n-p}.
\]
Rearranging, we arrive at the confidence interval,
\[
	\hat \beta_1 \pm t_{n-p}\left(\frac \alpha 2\right)\sqrt{\frac{(X^TX)^{-1}_{11}\hat\sigma^2 n}{n-p}}.
\]
...
\begin{align*}
	...
	H_1 &: \beta_1 \ne 0
\end{align*}
i.e. that the first predictor has no effect on the response. By the duality of confidence intervals and hypothesis tests, we can obtain a size $ \alpha $ test for $ H_0 $ by defining the acceptance region $ A = \{\beta:  0 \in I(\beta)\} $. In other words, accept $ H_0 $, if
\[
	|\hat \beta_1| < t_{n-p}\left(\frac \alpha 2\right)\sqrt{\frac{\hat\sigma^2 n (X^TX)^{-1}_{11}}{n-p}}.
\]
\begin{remark}
  THere is nothing special about testing $ \beta_1= 0 $: we could just as easily derive a size $ \alpha $ test for $ H_0: \beta_1 = \beta_1' $ for any $ \beta_1' $ in $ \R $.
\end{remark}
\begin{remark}
  We could also determine a confidence set (ellipsoid) for the whole vector of coefficients $ \beta $ by constructing a pivot based on \[
  \hat \beta-\beta = \mathcal N(0,\sigma^2(X^TX)^{-1})
  \]
  of which is explored in Example Sheet 3.
\end{remark}
\subsection{F-tests for linear regression}
Suppose we have a linear model $ Y = X\beta + \ep $, and assume we're in a normal model so $ \ep\sim\mathcal N (0,\sigma^2 I) $. We wish to test the null hypothesis that a whole collection of predictors have no effect on the response. This is the multidimensional case of the $ t $-test	 we just did previously. Without loss of generality, take $ p_0\le p $ coefficients. We want to test that
\begin{align*}
	H_0&: \beta_1 = \beta_2 = \cdots = \beta_{p_0} = 0 \\
	H_1 &: (\beta_1,\dots, \beta_{p_0}) \in \R^{p_0}.
\end{align*}
We write $ X = \begin{pmatrix}
	X_0 & X_1
\end{pmatrix}$ where $ X_0 $ is $ n\times p_0 $ and $ X_1 $ is $ n\times(p-p_0) $. Similarly we say that $ \beta = \begin{pmatrix}
   \beta^0 \\ \beta^1
\end{pmatrix} $. The null model has $ \beta^0 = 0 $ so it is a linear model
\[
  Y = X\beta + \ep = X_1 \beta^1 +\ep.
\]
We will write that $ P = X(X^TX)^{-1}X^T $ and $ P_1 = X_1(X_1^TX_1)^{-1}X_1^T $. Since $ X $ and $ P $ have full rank, so do $ X_1 $ and $ P_1 $.
\par
Recall that the MLE in the linear model is
\begin{align*}
	\max_{\beta\in \R^p,\sigma^2>0}\ell(\beta,\sigma^2) &= \ell(\hat\beta,\hat\sigma^2) = -\frac n2 \log\hat\sigma^2 - \frac{S(\hat\beta)}{(2\hat\sigma^2} + \text{const.}
							    &= - \frac n2 \log\left(\frac{||(I-P)Y||_2^2}n\right) + \text{const.}
\end{align*}
Thus a GLRT for our hypotheses ($ \beta^0 = 0 $ against $ \beta^0 \in \R^{p_0} $ involves computing
\begin{align*}
	\max_{\beta\in\R^P,\sigma^2>0}\ell(\beta,\sigma^2) - \max_{\beta^0 = 0, \sigma^2>0} \ell(\beat,\sigma^2) &= \frac n2 \left(-\log\left(\frac{||(I-P)Y||_2^2}n\right)+\log\left(\frac{||(I-P_1)Y||_2^2}n\right)\right).
\end{align*}
The GLRT rejects $ H_0 $ when this is large. This is monotone in
\begin{align*}
	\frac{||(I-P_1)Y||_2^2}{||(I-P)Y||_2^2} &= \frac{||(I-P+P-P_1)Y||_2^2}{||(I-P)Y||^2_2} \\
						&= \frac{||(I-P)Y||_2^2+||(P-P_1)Y||_2^2 + 2Y^T(I-P)(P-P_1)Y}{||(I-P)Y||_2^2}.
\end{align*}
\begin{lemma}
  We have that
  \[
	  (I-P)(P-P_1) = 0
  \]
and $ P-P_1 $ is an orthogonal projection with rank $ p_0 $.
\end{lemma}
\pf We know since $ P $ is a orthogonal projection that $ (I-P)P = 0 $. So we only have to look at $ (I-P)P_1 $. Suppose $ x\in \mathrm{col}(P_1) $ then $ x $ is also in the column space of $ P $, so
\[
	(I-P)P_1x = (I-P)x = x - Px = 0.
\]
If instead $ x\notin \mathrm{col}(P_1) $,
\[
	(I-P)P_1x = 0
\]
since $ P_1 $ is a orthogonal projection. So since true for all $ x $ we have that $ (I-P)(P-P_1) = 0 $.\par
Since $ P $ and $ P_1 $ are symmetric, $ P-P_1 $ is also symmetric. Since $ P_1 $ is a subprojection of $ P $, $ P_1P = PP_1 = P_1 $ so 
\begin{align*}
	(P-P_1)^2 = (P^2 - P_1P - PP_1 + P_1^2 = P - P_1
\end{align*}
hence $ P-P_1 $ is an orthogonal projection. Since $ P $ and $ P_1 $ have full rank they have traces $ p $ and $ p-p_0 $ respectively. Hence $ P-P_1 $ has trace and therefore rank $ p_0 $.\qed
\par
Thus the test statistic is monotonically increasing in
\begin{align*}
	\frac{||(P-P_1)Y||^2_2}{||(I-P)Y||^2_2} \frac{1/p_0}{1/(n-p)} = F
\end{align*}
Under $ H_0 $ we have that 
\[
(P-P_1)Y = (P-P_1)(X\beta + \ep) = (P-P_1)(X_1\beta^1 + \ep)= (P-P_1)\ep.
\]
Thus
\[
	F = \frac{||(P-P_1)\ep||^2_2}{||(I-P)\ep||^2_2} \frac{1/p_0}{1/(n-p)}.
\]
Observe that
\[
	\frac{||(P-P_1)\ep||^2_2}{\sigma^2} \sim \chi^2_p \quad \text{and}\quad \frac{||(I-P)\ep||^2_2}{\sigma^2} \sim \chi^2_{n-p}.
\]
And also the vector $ \begin{pmatrix}
	(P-P_1 ) \ep \\ (I-P)\ep
\end{pmatrix} $ is normal since it's an affine transformation of $ \ep $ and the covariance of $ (P-P1)\ep $ and $ (I-P)\ep $ is zero by the lemma. Thus we have that $ F\sim F_{p_0, n-p_0} $. So the generalised likelihood ratio test is a $ F $-test which rejects $ H_0 $ when
\[
	\frac{||P-P_1)Y||^2_2}{||(I-P)Y||_2^2} \cdot \frac{1/p_0}{1/(n-p)} > F_{p_0,n-p}(\alpha).
\]
\subsection{Logistic regression}
Suppose we observe data $ \{(x_1,Y_1),\dots, (x_n,Y_n)\} $ with $ Y_i\in \{0,1\} $ and $ x_i \in \R^p $. We wish to predict $ Y_i $ from $ x_i $. The \textit{logistic model} is
	\[
	p(x_i) = \mathbb P(Y_i\mid x_i) = \frac{\exp(\beta^T x_i)}{1+\exp(\beta^T x_i)}
	\]
	where $ \beta\in\R^p $ is the vector of parameters we want to estimate.\par
	The RHS is called the \textit{sigmoid funciton} of $ \beta^T x_i $. Rearranged, the model is
	\[
		\log\left(\frac{p(x_i)}{1-p(x_i)}\right) = \beta^T x_i.
	\]
Now we want to find the MLE. The likelihood can be written as
\begin{align*}
L(\beta) = \prod_{i:\ y_i = 1} p(x_i) \prod_{i\ y_i=0} (1-p(x_i)) &= \prod_{i=1}^n p(x_i)^{y_i}(1-p(x_i))^{1-y_i},
\end{align*}
and we wish to maximise
\[
	\ell(\beta) = \sum_{i=1}^n (y_u\log(p(x_i)) + (1-y_i)\log(1-p(x_i))).
\]
Based on an estimate $ \hat\beta $ of the logistic coefficients, we can compute
\[
	\hat p(x) = \frac{\exp(\hat\beta^T x)}{1+\exp(\hat\beta^T x)}
\]
on a new input, and classify $ x $ according to $ 1_{\{\hat p(x) > \frac 12\}} $.
