...
\end{remark}
\begin{theorem}
	SUppose $ T(X) $ is a statistics such that $ \frac{f_X(x\mid \theta)}{f_X(y\mid \theta)} $ is constnat a function of $ \theta $ if and only if $ T(x)=T(y) $. Then $ T $ is minimal statistics.
\end{theorem}
