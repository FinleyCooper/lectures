...
\[
	\left.\frac{\partial S(\beta)}{\partial \beta_k}\right|_{k=\beta} =0
\]
...
\begin{align*}
	...
	H_1 &: \beta_1 \ne 0
\end{align*}
i.e. that the first predictor has no effect on the response. By the duality of confidence intervals and hypothesis tests, we can obtain a size $ \alpha $ test for $ H_0 $ by defining the acceptance region $ A = \{\beta:  0 \in I(\beta)\} $. In other words, reject $ H_0 $, if
\[
	|\hat \beta_1| < t_{n-p}\left(\frac \alpha 2\right)\sqrt{\frac{\hat\sigma^2 n (X^TX)^{-1}_{11}}{n-m}}.
\]
\begin{remark}
