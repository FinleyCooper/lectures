...
\end{align*}
Now relabel $ n=-m $ in the second series and replace both contours $ \partial B(a,\rho_2) $ and $ \patrial B(a,\rho_1) $ by a common contour $ |w-a| = \rho $ using homotopy invariance of the integral. Thus we get the Laurent series.\par
...
\begin{theorem}
	(Local mapping degree) Let $ f:B(a,r)\to \C $ be holomorphic, non-constant and with $ \deg_{z=k}(f)=k>0 $. Then for all $ \rho>0 $ sufficiently small, there exists a $ \ep>0 $ such that if
	\[
	  0<|w-f(a)|<\ep
	\]
...
Pick some point $ y\in h(U) $. The open mapping theorem, says that there exists a $ \ep $ ball centred at $ y $. Now since $ \phi $ is injective, this implies that $ h(U)\cap B(-y,\ep)=\emptyst $ since if $ h(z_1) =\pm h(z_2) $ then $ z_1=z_2 $. Let's consider the map
\[
	z\to \frac{\ep}{2(h(z) + y)}
\]
