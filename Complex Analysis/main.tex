...
\end{remark}
\begin{theorem}
	(Rouche's Theorem) Let $ U\subseteq \C $ be a domain and let $ \gamma $ be a closed curve which bounds a domain $ D $ in $ U $. Let $ f,g $ be holomorphic on $ U $ and suppose that $ |f|>|g| $ on $ \gamma $. Then $ f $ and $ f+g $ have the same number of zeros in $ D $ when counter with multiplicity.
\end{theorem}
...
\]
Note that for all $ z\in \gamma $, $ h(z) \in B(1,1) $ so $ h\circ \gamma $ has imagei n $ \Re(z)>0 $ hence $ I(h\circ \gamma, 0) = 0 $. The the argument principle says that the number of zeros of $ h $ in $ S $ agree, counted with multiplicity. This is exactly what we want since zeros of $ h $ correspond to zeros of $ f+g $ and poles of $ h $ correspond to zeros of $ f $. \qed
\end{document}
