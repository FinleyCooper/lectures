...
\end{proposition}
\pf Recall that $ \rk(AB)\le \min(\rk(A),\rk(b)) $, so if either $ A $ or $ B $ has rank less than $ n $ then so does $ AB $ so by the corollary above $ \det(AB)=0=\det(A)\det(B) $. If not, $ A $ and $ B $ are invertible and can be written as a product of elementary matrices, so $ A=E_1\cdots E_\ell $, $ B+E_1'\cdots E_k' $. So $ AB=E_1\cdots E_\ell E_1'\cdots E_k' $, hence $ \det(AB)=\det(E_1)\cdots \det(E_\ell)\det(E_1')\cdots \det(E_k')=\det(A)\det(B)$.\qed
\end{document}
